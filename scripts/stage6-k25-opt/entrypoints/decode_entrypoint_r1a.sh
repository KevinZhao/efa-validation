#!/bin/bash
# Stage 6 R1a decode entrypoint — Kimi-K2.5 INT4 PD-1P1D, Mooncake KV (NO DeepEP).
#
...
#
# DeepEP stays OFF; Mooncake KV uses ALL 16 EFA rails (same as Stage 5 baseline).
#
...
echo "[launcher] Stage 6 R1a decode (KV=${KV_BACKEND}, a2a=none/DeepEP OFF, all 16 rails unified)"
...
echo "[launcher] starting sglang decode (Mooncake KV only, DeepEP OFF)"
