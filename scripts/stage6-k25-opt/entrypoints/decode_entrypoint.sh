#!/bin/bash
# Stage 6 R1 decode entrypoint — Kimi-K2.5 INT4 PD-1P1D, Mooncake KV + UCCL-EP a2a.
#
# Stacks 3 optimizations on top of the Stage 5 baseline
# (scripts/stage5-pd-1p1d-mc-vs-nixl/entrypoints/decode_entrypoint.sh):
...
#       gets the high 8 (rdmap87/88/112/113/137/138/162/163). Decode uses low_latency
#       mode. K2.5 top_k=8 ≤ UCCL-EP's hardcoded kNumMaxTopK=9 so UCCL-EP is feasible.
#
# KV_BACKEND is fixed to mooncake.
set -eu
...
echo "[launcher] Stage 6 R1 decode (KV=${KV_BACKEND}, a2a=deepep/low_latency via UCCL-EP)"
...
echo "[launcher] starting sglang decode (Mooncake KV + DeepEP-low_latency via UCCL-EP)"
