#!/bin/bash
# Nixl-only retry: reuse existing STAMP's raw dir (mooncake data preserved)
set -u
...
run_bench() {
    ...
    local runner; runner=$(get_prefill_pod)
    [[ -z "$runner" ]] && { log "no prefill; abort"; return 1; }
    ...
    kubectl -n "$NS" exec "$runner" -c server -- env HF_TOKEN="$HF_TOKEN" HUGGING_FACE_HUB_TOKEN="$HF_TOKEN" \
        python3 -m sglang.bench_serving --backend sglang --base-url "$LB_URL" --dataset-name random \
        --random-input-len "$inp" --random-output-len "$out" --num-prompts "$wu" --max-concurrency "$cc" \
        --model "$MODEL_LOCAL" --tokenizer "$MODEL_LOCAL" --disable-tqdm \
        > "$RESULTS/logs/${tag}-warmup.log" 2>&1 || log "warmup $tag non-zero (continuing)"
    kubectl -n "$NS" exec "$runner" -c server -- env HF_TOKEN="$HF_TOKEN" HUGGING_FACE_HUB_TOKEN="$HF_TOKEN" \
        python3 -m sglang.bench_serving --backend sglang --base-url "$LB_URL" --dataset-name random \
        --random-input-len "$inp" --random-output-len "$out" --num-prompts "$np" --max-concurrency "$cc" \
        --model "$MODEL_LOCAL" --tokenizer "$MODEL_LOCAL" --disable-tqdm --output-file "$raw_remote" > "$lg" 2>&1
    ec=$?
    if [[ $ec -ne 0 ]]; then log "bench $tag FAILED exit=$ec"; tail -30 "$lg" >> "$LOG" || true; return $ec; fi
    ...
    [[ -s "$RESULTS/raw/${tag}.json" ]] && log "bench $tag OK raw=$(stat -c %s "$RESULTS/raw/${tag}.json") bytes" || log "bench $tag: empty"
    sync_to_s3
}
...
teardown_wait() {
    ...
    while true; do
        ...
        if [ $(( $(date -u +%s) - start )) -gt 600 ]; then kubectl -n "$NS" delete pods --all --force --grace-period=0 2>/dev/null; sleep 10; return 0; fi
        sleep 10
    done
}
...
log "=== NIXL-ONLY RETRY start ==="
teardown_wait
apply_variant nixl || { log "APPLY nixl FAILED"; exit 1; }
log "=== SMOKE nixl ==="
