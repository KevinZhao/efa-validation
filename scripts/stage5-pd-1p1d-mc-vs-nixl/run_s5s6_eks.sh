#!/bin/bash
# S5/S6 long-context extension for existing A/B run.
# Appends 60K/1K and 120K/1K scenarios to stamp 20260501T002853Z so the final
# RESULT.md covers S1-S6 for both backends.
#
# Orchestration: mooncake first (full apply → run S5/S6 × 3 rounds → teardown),
# then switch to nixl, same. Reuses the same manifest/KV_BACKEND env mechanism
# as run_ab_eks.sh.
...
# ============= MAIN =============
log "=== S5/S6 LONG-CONTEXT EXTENSION START stamp=$STAMP ==="
