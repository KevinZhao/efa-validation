#!/usr/bin/env bash
# =============================================================================
# customer-1p1d-bench-ohio-dsv31.sh — Ohio p5en H200 x 2 run, DeepSeek-V3.1 block-FP8 (688 GB)
# 1P:1D perf comparison between uccl/nccl variants.
#
# Why: GLM-4.6 BF16 + UCCL-EP hit sglang 0.5.10 forward_deepgemm_{cont,masked}
# deprecated assert (BF16+deepep upstream-broken). DS-V3.1 is block-FP8 → hits
# deprecate_flag=True working path. top_k=8 ≤ UCCL kNumMaxTopK=9, 688 GB fits H200×8.
#
# For each variant:
...
#   5. save stats to results/customer-1p1d/<stamp>-ohio-dsv31/bench-<variant>.log
#   6. kubectl delete + 30s GPU drain before next variant
#
# Usage:
#   ./scripts/customer-1p1d-bench-ohio-dsv31.sh              # both uccl + nccl
#   VARIANT=uccl ./scripts/customer-1p1d-bench-ohio-dsv31.sh # only one
#
# Prereqs:
...
#   - S3 yanxi-validation-788668107894-ohio/models/deepseek-ai/DeepSeek-V3.1/ prefetched
#     (verified 2026-04-27: 533 objects / 688.6 GiB / .prefetch-complete sentinel)
# =============================================================================
set -euo pipefail
...
STAMP="$(ts)"
OUT_DIR="${REPO_ROOT}/results/customer-1p1d/${STAMP}-ohio-dsv31"
mkdir -p "${OUT_DIR}"

MANIFEST_SRC="${REPO_ROOT}/manifests/customer-1p1d-dsv31-ohio.yaml"
# Use Ohio bucket for manifest drops (bastion has access).
S3_BUCKET="yanxi-validation-788668107894-ohio"
S3_PREFIX="manifests/customer-1p1d-${STAMP}-ohio-dsv31"
