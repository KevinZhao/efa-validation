# Mooncake (含 Henan 5 EFA PR) vs NIXL 性能对比
...
- Mooncake upstream @`634b7097` = v0.3.10.post2 + **王鹤男（Henan）5 个 EFA PR**
...
## 6. Henan 5 个 EFA PR 清单（Mooncake 侧改动）

**仓库**：`mooncake-labs/mooncake`
**版本锁定**：`v0.3.10.post2` = upstream @`634b7097` + 以下 PR

| PR    | 作用                              |
|-------|-----------------------------------|
| #1944 | EFA RDMA provider (rdma→efa 映射) |
| #1912 | PTE-aware memory registration     |
| #1911 | EFA send-completion 优化          |
| #1903 | Auto-split 大 transfer            |
| #1899 | libfabric/EFA context init        |
