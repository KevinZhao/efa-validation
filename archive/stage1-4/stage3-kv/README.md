...
对齐 [`../../EFA_Validation_Plan.md`](../../EFA_Validation_Plan.md) 4.3 节。

## 目标

| 路径 | 定位 | 判据 |
|------|------|------|
| **Mooncake Transfer Engine + `efa` 协议** | **主**（客户生产栈 KV 层） | 2 节点跨机 put/get，**单节点聚合吞吐 ≥ 150 GB/s，目标 ≈ 170 GB/s**（8× EFA） |
...
```
788668107894.dkr.ecr.us-east-2.amazonaws.com/yanxi/mooncake-nixl:v1
788668107894.dkr.ecr.us-east-2.amazonaws.com/yanxi/mooncake-nixl:latest
```
...
```bash
kubectl -n yanxi-validation get mpijob
kubectl -n yanxi-validation logs -f job/mooncake-bench-efa-launcher
...
```bash
kubectl -n yanxi-validation cp \
    <launcher-pod>:/tmp/ \
    ./logs/stage3-kv/ \
    -c launcher
aws s3 sync ./logs/stage3-kv/ s3://yanxi-validation-788668107894/logs/stage3-kv/
```
...
| NIXL 对照 | 同 payload 下带宽相对 Mooncake 的位置 | 给客户一个外部参照数字；若反超，也只是 fallback 的利好 |
...
- [ ] **etcd 版本**：Launcher 先用 `dependencies.sh` 安装的 etcd，否则 fallback 拉 `v3.5.13`
  release 二进制。如果客户环境禁止出网，需要预先把 etcd 打进镜像（TODO: 下一轮加到 Dockerfile）。
